...
from integrations.mootdx_source import fetch_stock_mootdx as fetch_stock_mootdx
...
def _try_mootdx(ctx: StockHistFetchContext) -> pd.DataFrame | None:
    if _env_flag("DATA_SOURCE_DISABLE_MOOTDX"):
        ctx.failed_details.append("mootdx=disabled_by_env")
        return None
    try:
        df = fetch_stock_mootdx(ctx.symbol, ctx.start_s, ctx.end_s, ctx.adjust)
        return integrations.data_source_format.tag_source(df, "mootdx")
    except ModuleNotFoundError as exc:
        ctx.failed_details.append(f"mootdx=not_installed:{getattr(exc, 'name', '')}")
        _debug_source_fail("mootdx", exc)
    except Exception as exc:
        ctx.failed_details.append(f"mootdx={integrations.data_source_format.compact_error(exc)}")
        _debug_source_fail("mootdx", exc)
    return None
...
def _stock_hist_failure(ctx: StockHistFetchContext) -> RuntimeError:
    ...
    return RuntimeError(
        ...
        f"mootdx→tickflow→tushare→akshare→baostock→efinance，均无可用 K 线数据。请检查该标的是否已退市或处于长期停牌期。"
